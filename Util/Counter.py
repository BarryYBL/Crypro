...
The block ciphers from `Crypro.Cipher` (when configured in *MODE_CTR* mode)
...
by the ciphers in `Crypro.Cipher`. The fact that the Python layer is bypassed
...
from Crypro.Util import _counter
import struct
...
def new(nbits, prefix=b(""), suffix=b(""), initial_value=1, overflow=0, little_endian=False, allow_wraparound=False, disable_shortcut=False):
    ...
    :Parameters:
      ...
      disable_shortcut : boolean
        If True, do not make ciphers from `Crypro.Cipher` bypass the Python
